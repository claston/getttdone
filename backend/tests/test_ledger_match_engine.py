from app.application.ledger_match_engine import match_exact_then_date_tolerance_1to1
from app.application.models import NormalizedTransaction


def test_match_exact_then_date_tolerance_1to1_matches_only_same_date_and_amount() -> None:
    bank_rows = [
    ...
    result = match_exact_then_date_tolerance_1to1(bank_rows=bank_rows, sheet_rows=sheet_rows)
...
def test_match_exact_then_date_tolerance_1to1_does_not_reuse_sheet_row() -> None:
    bank_rows = [
        NormalizedTransaction(date="2026-04-01", description="A", amount=-100.0, type="outflow"),
...
def test_match_exact_then_date_tolerance_1to1_matches_within_plus_or_minus_two_days() -> None:
    bank_rows = [
...
def test_match_exact_then_date_tolerance_1to1_does_not_match_outside_two_day_window() -> None:
    bank_rows = [
        NormalizedTransaction(date="2026-04-05", description="A", amount=-100.0, type="outflow"),
    ]
    sheet_rows = [
        NormalizedTransaction(date="2026-04-02", description="X", amount=-100.0, type="outflow"),
    ]
...
def test_match_exact_then_date_tolerance_1to1_prioritizes_exact_before_tolerance() -> None:
    bank_rows = [
        NormalizedTransaction(date="2026-04-03", description="A", amount=-100.0, type="outflow"),
    ]
    sheet_rows = [
        NormalizedTransaction(date="2026-04-01", description="X", amount=-100.0, type="outflow"),
        NormalizedTransaction(date="2026-04-03", description="Y", amount=-100.0, type="outflow"),
    ]
    ...
    assert result.bank_unmatched_count == 0
    assert result.sheet_unmatched_count == 1
    assert result.matches[0].sheet_index == 1
    assert result.matches[0].match_rule == "exact"
